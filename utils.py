...
def b_cubed(predicted_clusters, gold_clusters):
    ...
    for mention in mentions:
        ...
        precisions.append(len(corrects) / len(mention2predicted_cluster) if len(mention2predicted_cluster) > 0 else 0) 
        recalls.append(len(corrects) / len(mention2gold_cluster) if len(mention2gold_cluster) > 0 else 0)
...
def ceaf(predicted_clusters, gold_clusters):
    ...
    precision = max_correct_mentions / len(sum(predicted_clusters, [])) if len(sum(predicted_clusters, [])) > 0 else 0
    recall = max_correct_mentions / len(sum(gold_clusters, [])) if len(sum(gold_clusters, [])) > 0 else 0
    f1 = get_f1(precision, recall)
    return precision, recall, f1
